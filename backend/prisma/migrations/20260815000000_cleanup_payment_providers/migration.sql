...
UPDATE "webhook_events" SET "provider" = 'MANUAL' WHERE "provider" IN ('WAVE', 'CINETPAY', 'ORANGE_MONEY', 'MTN_MOMO');

-- 2. Drop defaults before swapping the enum type
ALTER TABLE "transactions" ALTER COLUMN "provider" DROP DEFAULT;

-- 3. Swap enum: rename old → create new → cast ALL columns → drop old
...
ALTER TABLE "webhook_events" ALTER COLUMN "provider" TYPE "PaymentProvider" USING ("provider"::text::"PaymentProvider");
DROP TYPE "PaymentProvider_old";

-- 4. Re-set the default
