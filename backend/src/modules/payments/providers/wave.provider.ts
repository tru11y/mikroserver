...
export class WaveProvider implements IPaymentProvider {
  ...
  constructor(private readonly configService: ConfigService) {
    const apiUrl = this.configService.get<string | undefined>("wave.apiUrl");
    const apiKey = this.configService.get<string | undefined>("wave.apiKey");
    this.webhookSecret = this.configService.get<string | undefined>(
      "wave.webhookSecret",
    );
    this.allowedIps = this.configService.get<string[]>("wave.allowedIps", []);
    this.enabled = Boolean(apiUrl && apiKey && this.webhookSecret);

    if (!this.enabled) {
      this.logger.warn("Wave provider not configured — provider disabled");
      return;
    }
    ...
    axiosRetry(this.httpClient, {
      ...
      onRetry: (retryCount, error) => {
        this.logger.warn(`Wave API retry ${retryCount}/3: ${error.message}`);
      },
    });
  }
  ...
  async createPayment(input: CreatePaymentInput): Promise<CreatePaymentOutput> {
    ...
    const successUrl =
      input.successUrl ||
      this.configService.get<string | undefined>("wave.successUrl");
    const errorUrl =
      input.errorUrl ||
      this.configService.get<string | undefined>("wave.errorUrl");
    const currency = this.configService.get<string>("wave.currency", "XOF");

    if (!successUrl || !errorUrl) {
      throw new ServiceUnavailableException(
        "Wave return URLs are not configured",
      );
    }
    ...
    const response = await httpClient.post<WaveCheckoutResponse>(
      "/checkout/sessions",
      payload,
      {
        headers: {
  ...
  async verifyAndParseWebhook(
    ...
    if (!input.signature) {
      this.logger.warn(
        `Wave webhook missing signature from IP: ${input.ipAddress}`,
      );
    ...
    // 3. Parse payload
    const parsed: WaveWebhookPayload = JSON.parse(
      input.rawBody,
    ) as WaveWebhookPayload;
  ...
  async getPaymentStatus(externalReference: string): Promise<PaymentStatus> {
    const response = await this.getHttpClient().get<WaveCheckoutResponse>(
  ...
  private getHttpClient(): AxiosInstance {
    if (!this.enabled || !this.httpClient) {
      throw new ServiceUnavailableException("Wave provider is not configured");
    }
