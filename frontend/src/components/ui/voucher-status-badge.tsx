...
import { getVoucherStatusCfg } from '@/lib/voucher-status';
...
export function VoucherStatusBadge({ status, className }: VoucherStatusBadgeProps) {
  const cfg = getVoucherStatusCfg(status);
  const Icon = cfg.icon;
  return (
    <span
      className={clsx(
    ...
    >
      {cfg.pulse ? (
        <span className="relative flex h-1.5 w-1.5 flex-shrink-0" aria-hidden="true">
          <span className={clsx('absolute inline-flex h-full w-full animate-ping rounded-full opacity-75', cfg.dot)} />
          <span className={clsx('relative inline-flex h-1.5 w-1.5 rounded-full', cfg.dot)} />
        </span>
      ) : (
