...
export function RouterAlertMonitor() {
  ...
  const routers: Router[] = (data?.data?.data as Router[]) ?? [];
  ...
  useEffect(() => {
    ...
    routers.forEach((router) => {
      ...
      } else if (prev === 'OFFLINE' && router.status === 'ONLINE') {
        // Router came back → green toast
        toast.success(`🟢 ${router.name} est de retour en ligne`, {
          description: 'Le routeur répond à nouveau.',
          duration: 6_000,
