...
object RoutersTable : Table("routers") {
    ...
    val wgAllowedIp = text("wg_allowed_ip")
    val wgEndpoint = text("wg_endpoint").nullable()
    val dnatPortBase = integer("dnat_port_base")
    ...
    val status = text("status").default("PROVISIONING")
    val lastHandshakeAt = timestamp("last_handshake_at").nullable()
    val provisionedAt = timestamp("provisioned_at").nullable()
    val createdAt = timestamp("created_at")
    val updatedAt = timestamp("updated_at")
    val deletedAt = timestamp("deleted_at").nullable()
    override val primaryKey = PrimaryKey(id)
